...
def _set_resource_only(monkeypatch, resource="test-resource"):
...
def _clear_all_foundry_env(monkeypatch):
...
def test_bare_path_implies_review_command(tmp_path):
...
def test_main_inserts_review_when_no_known_subcommand(monkeypatch, tmp_path):
    repo = make_git_repo(tmp_path)
...
def test_main_routes_commit_subcommand(monkeypatch, tmp_path):
    repo = make_git_repo(tmp_path)
...
def test_main_routes_heal_subcommand(tmp_path):
    repo = make_git_repo(tmp_path)
    with pytest.raises(SystemExit) as exc_info:
...
def test_resolve_git_repo_rejects_nonexistent_path(tmp_path):
    with pytest.raises(SystemExit) as exc_info:
        cli._resolve_git_repo(str(tmp_path / "does_not_exist"))
    assert "is not a directory" in str(exc_info.value)


def test_resolve_git_repo_rejects_non_git_directory(tmp_path):
...
def test_review_without_api_key_fails_cleanly_not_a_traceback(monkeypatch, tmp_path):
    repo = make_git_repo(tmp_path)
...
def test_review_without_resource_fails_cleanly(monkeypatch, tmp_path):
    repo = make_git_repo(tmp_path)
    _clear_all_foundry_env(monkeypatch)
    parser = cli.build_review_parser()
    args = parser.parse_args(["review", "--path", str(repo)])
...
def test_use_entra_id_flag_defaults_to_none_and_parses_true(tmp_path):
    repo = make_git_repo(tmp_path)
    parser = cli.build_review_parser()
...
def test_model_flag_defaults_to_none_so_the_env_var_can_apply(tmp_path):
...
def test_build_reviewer_uses_the_model_env_var_when_no_flag_is_given(monkeypatch, tmp_path):
    repo = make_git_repo(tmp_path)
    _set_resource_only(monkeypatch)
...
def test_build_reviewer_model_flag_overrides_the_env_var(monkeypatch, tmp_path):
    repo = make_git_repo(tmp_path)
    _set_resource_only(monkeypatch)
    monkeypatch.setenv("ANTHROPIC_FOUNDRY_API_KEY", "fake-key")
    monkeypatch.setenv("ANTHROPIC_FOUNDRY_MODEL", "claude-haiku-4-5")
...
_FAKE_RUNNER = discovery.TestRunner(name="pytest", command=["pytest"])
_FAKE_FAILING_RESULT = healing.TestRunResult(
    runner=_FAKE_RUNNER, returncode=1, stdout="", stderr="assert 1 == 2"
)
_FAKE_PASSING_RESULT = healing.TestRunResult(
    runner=_FAKE_RUNNER, returncode=0, stdout="", stderr=""
)
_FAKE_PROPOSAL = healing.HealingProposal(
...
def _configure_heal_env(monkeypatch):
    _set_resource_only(monkeypatch)
    monkeypatch.setenv("ANTHROPIC_FOUNDRY_API_KEY", "fake-key")


def test_heal_without_apply_never_calls_apply_patch(monkeypatch, tmp_path):
...
def test_heal_with_apply_flag_writes_and_reruns(monkeypatch, tmp_path):
    repo = make_git_repo(tmp_path)
    _configure_heal_env(monkeypatch)
    run_tests_calls = []

    def _fake_run_tests(repo, runner=None):
...
def test_cmd_init_scaffolds_target_repo(tmp_path):
    repo = make_git_repo(tmp_path)
...
def test_main_init_is_idempotent(tmp_path, capsys):
    repo = make_git_repo(tmp_path)
...
# -- _parse_fail_on (milestone 1: --fail-on's severity threshold parsing) --


def test_parse_fail_on_none_means_no_gating():
    assert cli._parse_fail_on(None) is None


def test_parse_fail_on_single_severity_each_rank():
    assert cli._parse_fail_on("critical") == 0
    assert cli._parse_fail_on("high") == 1
    assert cli._parse_fail_on("medium") == 2
    assert cli._parse_fail_on("low") == 3


def test_parse_fail_on_is_case_insensitive():
    assert cli._parse_fail_on("High") == cli._parse_fail_on("high")
    assert cli._parse_fail_on("CRITICAL") == cli._parse_fail_on("critical")


def test_parse_fail_on_multiple_severities_uses_the_least_severe_rank():
    # "critical,high" means "at or above high" -- equivalent to "high"
    # alone, since high already covers critical.
    assert cli._parse_fail_on("critical,high") == cli._parse_fail_on("high")
    assert cli._parse_fail_on("high,critical") == cli._parse_fail_on("high")  # order-independent
    assert cli._parse_fail_on("low,critical") == cli._parse_fail_on("low")


def test_parse_fail_on_tolerates_whitespace_around_commas():
    assert cli._parse_fail_on(" high , critical ") == cli._parse_fail_on("high")


def test_parse_fail_on_empty_string_raises():
...
def test_parse_fail_on_unknown_severity_raises():
    with pytest.raises(ValueError, match="unknown severity"):
        cli._parse_fail_on("extreme")


def test_parse_fail_on_one_bad_value_among_good_ones_still_raises():
    with pytest.raises(ValueError, match="unknown severity"):
        cli._parse_fail_on("high,extreme")
